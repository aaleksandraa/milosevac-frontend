...
APP_ROOT="${APP_ROOT:-/var/www/vhosts/milosevac.com/httpdocs}"
FRONTEND_DIR="${FRONTEND_DIR:-$APP_ROOT}"
REF="${1:-main}"
NPM_BIN="${NPM_BIN:-npm}"
...
(
  cd "$FRONTEND_DIR"
  "$NPM_BIN" ci
  VITE_BACKEND_PUBLIC_URL="${VITE_BACKEND_PUBLIC_URL:-https://api.milosevac.com}" "$NPM_BIN" run build
)
